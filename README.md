...
The GitHub Actions workflow in `.github/workflows/deploy-cloudflare-pages.yml` deploys pushes to `main` and PRs when the repository has `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, and `CLOUDFLARE_PAGES_PROJECT_NAME` configured as GitHub Actions secrets. Cloudflare Git integration settings do not populate these GitHub secrets.
